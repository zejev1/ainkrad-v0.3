...
describe('0.3.22 F2 lived shipbuilding progression',()=>{
  it('requires real prior vessel experience before a Spark can improve the design',async()=>{
    ...
    a.locationId='rulid_shipyard';a.position={x:world.places.rulid_shipyard.mapX,y:world.places.rulid_shipyard.mapY,layerId:'surface'};delete a.movement;
    a.knownPlaceIds=[...(a.knownPlaceIds??[]),'rulid_shore','rulid_shipyard'];
    ...
    expect(boat.locationId).toBe('rulid_shipyard');
    ...
    expect(projects[0].locationId).toBe('rulid_shipyard');
  });
});
