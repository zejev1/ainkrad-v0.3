...
describe('Release provenance gate', () => {
  it('rejects reused release numbers and advances fix, build and lockfile together', () => {
    ...
    try {
      ...
      const metadata = () => JSON.parse(run('node', ['--input-type=module', '-e', 'import { releaseMetadata } from "./scripts/release.mjs"; console.log(JSON.stringify(releaseMetadata()));']));
      expect(metadata().dirty).toBe(false);
      mkdirSync(join(root, '.vercel', 'output'), { recursive: true });
      writeFileSync(join(root, '.vercel', 'output', 'config.json'), '{}');
      expect(metadata().dirty).toBe(false);
