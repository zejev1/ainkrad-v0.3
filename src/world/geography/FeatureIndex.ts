...
export class FeatureIndex<T> {
  ...
  private itemBounds:FeatureBounds[];
  constructor(readonly items:readonly T[],private bounds:(item:T)=>FeatureBounds,private size=512) {
    this.itemBounds=items.map(item=>({...bounds(item)}));
    items.forEach((item,id)=>{const b=this.itemBounds[id],x0=Math.floor(b.minX/size),x1=Math.floor(b.maxX/size),y0=Math.floor(b.minY/size),y1=Math.floor(b.maxY/size);
  ...
  query(b:FeatureBounds):T[] {
    ...
    if((x1-x0+1)*(y1-y0+1)>1024)return this.items.filter((_item,id)=>overlap(b,this.itemBounds[id]));
