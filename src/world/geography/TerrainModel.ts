...
import {clamp,lerp} from './TerrainMath';
...
export class TerrainModel {
  readonly outline:WorldPoint2D[];readonly ocean:WorldPoint2D[];readonly ranges;
  ...
  constructor(readonly foundation:TerrainFoundation) {
    ...
    this.ocean=[...outer,outer[0],this.outline[0],...this.outline.slice(1),this.outline[0],outer[0]];
  ...
  sample(x:number,y:number,detail=true):TerrainSample {
    ...
    if(!pointInPolygon(p,this.outline))return {height:-100,moisture:1,biome:'ocean',water:true,slope:0};
