...
export function segmentEntersBuilding(a: WorldPoint2D, b: WorldPoint2D, place: WorldPlace): boolean {
  const size=buildingSize(place);if(!size.width)return false;
  const start=buildingLocalPoint(a,place),end=buildingLocalPoint(b,place);
  let enter=0,leave=1;
  for(const [origin,delta,half]of [[start.x,end.x-start.x,size.width/2],[start.y,end.y-start.y,size.height/2]]) {
    const r=half+STREET_CLEARANCE-1e-6;
    if(Math.abs(delta)<1e-10) {if(origin<=-r||origin>=r)return false;}
    else {
      const t1=(-r-origin)/delta,t2=(r-origin)/delta;
...
export function routeAroundBuildings(
  ...
  const nodes = [path[0], path.at(-1)!, ...buildings.flatMap(p => buildingPolygon(p, STREET_CLEARANCE + .003))];
...
export function insideWater(point: WorldPoint2D, water: readonly WorldPlace[]): boolean {
  return water.some(place => {
    const polygon = place.waterPolygon ?? place.boundaryPolygon;
...
export function vacantHomePlot(places: Readonly<Record<string, WorldPlace>>, preferred: WorldPoint2D): WorldPoint2D | undefined {
  ...
  const water = Object.values(places).filter(p => p.surface === 'water' || p.waterPolygon);
...
export function dryBuildingPlot(point: WorldPoint2D, halfX: number, halfY: number, water: readonly WorldPlace[], rotation=0): boolean {
  const c=Math.cos(rotation),s=Math.sin(rotation);
  const corners=[[-halfX,-halfY],[halfX,-halfY],[halfX,halfY],[-halfX,halfY]]
    .map(([x,y])=>({x:point.x+x*c-y*s,y:point.y+x*s+y*c}));
  return !water.some(place=>{
    const polygon=place.waterPolygon??place.boundaryPolygon;
    return polygon&&polygonsOverlap(corners,polygon);
  });
}
