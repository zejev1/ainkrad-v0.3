...
export function renderCardinalControl(control: CardinalControlSnapshot | undefined, worldMinutes: number): void {
  ...
  const display = `${control.status}:${worldMinutes < 200 * 525600}:${control.recoveries}:${control.attempts}:${control.error ?? ''}`;
  ...
  status.textContent = control.status === 'OFF' ? 'Кардинал OFF · наблюдение и управление отключены. Мир работает самостоятельно.'
    ...
    : 'Кардинал ONLINE · погодный агент под наблюдением; прочие полномочия ограничены правилами мира.';
  status.title = control.error ?? '';
  document.getElementById('cardinal-control-summary')!.textContent =
    `Восстановлений погоды: ${control.recoveries}.` + (control.attempts >= 3 ? ' Повторные попытки остановлены; требуется проверка.' : '');
  }
  const sequence = control.recent.at(-1)?.sequence ?? 0;
  if (lastSequence === sequence) return;
  lastSequence = sequence;
  const list = document.getElementById('cardinal-system-log')!;
  list.replaceChildren();
  for (const record of [...control.recent].reverse()) {
    const row = document.createElement('li');
    row.textContent = `Год ${(record.minute / 525600).toFixed(2)} · ${labels[record.kind]}`;
