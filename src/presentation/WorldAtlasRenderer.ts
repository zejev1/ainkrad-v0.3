...
export class WorldAtlasRenderer {
  ...
  private paintTerrain(camera:Readonly<WorldMapCamera>,world:Readonly<WorldState>):void {
    ...
    if(model){
      ...
      const outlines=model.landOutlines.map(poly=>clipMapPolygon(poly.map(p=>camera.point(p.x,p.y)))).filter(poly=>poly.length>2);
      path.setAttribute('d',outlines.map(poly=>'M'+poly.map(p=>p.x+' '+p.y).join('L')+'Z').join(' ')||'M0 0');clip.replaceChildren(path);
    ...
    if(model){paintRegionalFeatures(model,camera,this.shapes);this.details.render(model,camera,world);}
