...
export class WorldAtlasRenderer {
  ...
  constructor(private ground:HTMLElement,private roads:SVGSVGElement,private towns:HTMLElement) {
    ...
    this.shapes=document.createElementNS(SVG,'g');this.svg.append(this.shapes);ground.replaceChildren(this.svg);towns.replaceChildren();
  }
  render(world:Readonly<WorldState>,camera:Readonly<WorldMapCamera>):void {
    ...
    if(cameraKey!==this.terrainKey) {this.paintTerrain(camera);this.terrainKey=cameraKey;}
  ...
  private paintTerrain(camera:Readonly<WorldMapCamera>):void {
    ...
    for(const area of areas) {
      ...
      path.dataset.area=area.id;fragment.append(path);
      if(area.kind==='water') {
