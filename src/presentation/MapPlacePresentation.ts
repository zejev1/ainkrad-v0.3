...
import type { CoastalLandmark } from './CoastalLandmark';

export function isPersistentMapLandmark(place:Readonly<WorldPlace>):boolean {
  return place.id === 'rulid_shore';
}

export function physicalPlaceDrawing(place:Readonly<WorldPlace>,close:boolean,coast?:CoastalLandmark):string {
  if (place.id === 'rulid_shore' && coast) return coast.svg;
...
export function applyPhysicalPlaceStyle(element:HTMLElement,place:Readonly<WorldPlace>,camera:Readonly<WorldMapCamera>,coast?:CoastalLandmark):void {
  element.classList.toggle('is-coastal-geometry', Boolean(coast));
  if (coast) {
    const { minX, minY, maxX, maxY } = coast.bounds;
    element.style.setProperty('--physical-width', (maxX - minX) * camera.pixelsPerUnit + 'px');
    element.style.setProperty('--physical-height', (maxY - minY) * camera.pixelsPerUnit + 'px');
    element.style.setProperty('--coastal-shift-x', ((minX + maxX) / 2 - coast.origin.x) * camera.pixelsPerUnit + 'px');
    element.style.setProperty('--coastal-shift-y', ((minY + maxY) / 2 - coast.origin.y) * camera.pixelsPerUnit + 'px');
    element.classList.remove('is-town-pin', 'is-natural-marker');
    return;
  }
...
export function visibleMapLabels(places:readonly WorldPlace[],camera:Readonly<WorldMapCamera>,highlighted:ReadonlySet<string>,
  ...
  const sorted=[...places].sort((a,b)=>Number(highlighted.has(b.id))-Number(highlighted.has(a.id))||
    Number(isPersistentMapLandmark(b))-Number(isPersistentMapLandmark(a))||
