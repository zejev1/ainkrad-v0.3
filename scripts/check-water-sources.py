"""Independent, read-only source audit. It never changes water coefficients or a world.
Usage: python scripts/check-water-sources.py [--record]
...
MANIFEST = ROOT / 'data' / 'water-source-audit.json'
ALLOWED = {'www.usgs.gov', 'www.fao.org'}
...
if __name__ == '__main__':
    ...
    for row in rows:
        print(row['id'], row['status'], 'changed' if row.get('changedSincePreviousFetch') else '')
