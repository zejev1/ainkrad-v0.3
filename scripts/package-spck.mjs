...
const base='d2a3463540e5b931847f2b09980ea6df6b16b884';
...
assert.equal(report.success,true);assert.equal(report.numFailedTests,0);assert(report.numPassedTests>=266);
assert(statSync(join(root,'dist','index.html')).isFile(),'Production build is missing.');
const benchmark=JSON.parse(readFileSync(join(temp,'ainkrad-fix4-benchmark.json'),'utf8'));
assert.equal(benchmark.baseline,base);
assert.equal(benchmark.ioProbe,'passed');
assert.equal(benchmark.worldHistoryEqualExceptRevision,true);
assert.equal(benchmark.originalQueueYearsAfterSlowdown,100);
assert.equal(benchmark.browserQueueBounded,true);
assert(benchmark.fixedSeconds<benchmark.baselineSeconds);
...
const audit={
  release:'0.3.21-hotfix.4',status:'passed',base_commit:base,tested_commit:head,
  ...
  review:'docs/V0_3_21_FIX4_REVIEW.md',
  ...
  limitations:['No real Android or Xbox run','Original device save was not supplied; clock cancellation and continuity use compatible synthetic saves',
    '100 years per minute is a requested ceiling, not guaranteed throughput; acceleration while closed requires opt-in',
    'Different origins retain separate worlds','Three same-database backups cannot survive deletion of all browser site data'],
};
writeFileSync(join(target,'docs','V0_3_21_FIX4_AUDIT.json'),JSON.stringify(audit,null,2)+'\n');
const hashes={};
for(const path of paths) {
...
writeFileSync(join(target,'docs','V0_3_21_FIX4_FILES.json'),JSON.stringify({testedCommit:head,sha256:hashes},null,2)+'\n');
